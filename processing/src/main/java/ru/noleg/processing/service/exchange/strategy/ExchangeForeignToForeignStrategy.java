...
@Component
public class ExchangeForeignToForeignStrategy implements CurrencyExchangeStrategy {

    private final AccountService accountService;
    private final CurrencyServiceClient currencyService;

    public ExchangeForeignToForeignStrategy(AccountService accountService, CurrencyServiceClient currencyService) {
    ...
    public BigDecimal exchange(UUID uuid, Account source, Account target, BigDecimal amount) {
        BigDecimal rateFrom = currencyService.fetchCurrencyRate(source.getCurrencyCode());
        BigDecimal rateTo = currencyService.fetchCurrencyRate(target.getCurrencyCode());

        accountService.addMoneyToAccount(uuid, source.getId(), amount.negate());

        BigDecimal rubAmount = amount.multiply(rateFrom);
        BigDecimal result = rubAmount.divide(rateTo, 4, RoundingMode.HALF_DOWN);
